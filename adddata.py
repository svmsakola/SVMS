    ...
    for r in results:
        if r.boxes is not None:
            for box in r.boxes.data:
                ...
                b = buf.tobytes()
                filename = f"{FACEDATA_PREFIX}/{uid}/{uid}_img{len(saved_images) + 1}.jpg"
                ...
                if encs:
                    ...
                    np.save(bio, arr)
                    bio.seek(0)
                    encoding_filename = f"{ENCODINGS_PREFIX}/{uid}_encoding{len(face_encodings) + 1}.npy"
                ...
                for box in fr.boxes.data:
                    ...
                    for uid, user in known_data.items():
                        ...
                        for encoding_path in user.get("face_encodings", []):
                            try:
                                b = s3_get_bytes(encoding_path)
                                arr = np.load(io.BytesIO(b))
                                user_encodings.append(arr)
                        ...
                        if not user_encodings:
                            ...
                            for idx, image_path in enumerate(images):
                                try:
                                    b = s3_get_bytes(image_path)
                                    img = face_recognition.load_image_file(io.BytesIO(b))
                                    face_encs = face_recognition.face_encodings(img, model=face_recognition_model)
                                    if not face_encs:
                                        continue
                                    ...
                                    bio.seek(0)
                                    encoding_path = f"{ENCODINGS_PREFIX}/{uid}_{idx}.npy"
                                    s3_put_bytes(encoding_path, bio.read())
                    ...
                    if face_encodings:
                        ...
                        if True in matches:
                            ...
                            return jsonify({"recognized": True, "user_data": matched_user, "uid": matched_uid})
        if not face_found:
...
def process_profile_image():
    ...
    try:
        ...
        _, buf = cv2.imencode(".jpg", face_crop)
        b = buf.tobytes()
        key = f"{PROFILES_PREFIX}/{uid}/{uid}_img{next_img_num}.jpg"
...
def generate_pdf():
    ...
    try:
        ...
        pdf_key = f"{PDF_PREFIX}/{pdf_filename}"
        buffer = io.BytesIO()
...
def cleanup_images():
    try:
        resp = s3.list_objects_v2(Bucket=BUCKET, Prefix=UPLOAD_PREFIX)
...
def serve_remark_pdf(filename):
    key = f"{REMARK_PDF_PREFIX}/{filename}"
    try:
        b = s3_get_bytes(key)
...
def delete_user():
    ...
    try:
        ...
        profile_prefix = f"{PROFILES_PREFIX}/{uid_to_delete}/"
        resp = s3.list_objects_v2(Bucket=BUCKET, Prefix=profile_prefix)
        for obj in resp.get("Contents", []):
            s3_delete(obj["Key"])
        face_data_prefix = f"{FACEDATA_PREFIX}/{uid_to_delete}/"
...
def get_registration_image(uid, filename):
    key = f"{FACEDATA_PREFIX}/{uid}/{filename}"
    if not s3_exists(key):
        alt_key = f"{FACEDATA_PREFIX}/{filename}"
