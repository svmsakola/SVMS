                    ...
                    if pdf_path and isinstance(pdf_path, str):
                        ...
                        if abs_pdf_path.startswith(abs_pdf_folder) and os.path.exists(abs_pdf_path):
                            try:
                                ...
                                def is_admin_authenticated():
                                    print(
                                        "WARNING: delete_visitor_entry endpoint called WITHOUT proper authentication check!")
                                    return True
                                    ...
                                    if request.method == "POST":
                                        if request.is_json:
                                            ...
                                            if username in users and bcrypt.checkpw(password.encode('utf-8'),
                                                ...
                                                session["user"] = {"username": username, "role": role}
                                                return jsonify(success=True,
                                                               redirect_url=url_for("dashboard", role=role))
                                ...
                                def register_visitor():
                                    ...
                                    for uid_key, user_data_val in facedata.items():
                                        if isinstance(user_data_val, dict):
                                            if user_data_val.get('name') == data.get('name') and user_data_val.get(
                                                    'phone') == data.get('phone'):
                                                existing_uid = uid_key
                                                break
                                        else:
                                            print(
                                                f"Warning: Skipping non-dict entry for UID {uid_key} in facedata.json")

                                    uid = existing_uid if existing_uid else f"UID{int(datetime.now().timestamp())}"

                                    tahasil_options = ["अकोला", "अकोट", "तेल्हारा", "बाळापूर", "पातूर", "मुर्तिजापूर",
                                                       "बार्शीटाकळी"]
                                    selected_tahasil = data.get('tahasil', '')
                                    if selected_tahasil.endswith('<'):
                                        selected_tahasil = selected_tahasil[:-1]
                                    ...
                                    if not existing_uid:
                                        visitor_data['registration_datetime'] = datetime.now().strftime(
                                            "%Y-%m-%d %H:%M:%S")
                                        if 'frame' in data and data['frame']:
                                            try:
                                                frame_data = data['frame']
                                                ...
                                                if frame is not None:
                                                    ...
                                                    return jsonify(
                                                        {'error': 'Invalid frame data, unable to decode'}), 400
                                            except Exception as e:
                                                return jsonify({'error': f'Error processing frame: {str(e)}'}), 500
                                            finally:
                                                cleanup_memory()
                                    ...
                                    if existing_uid and uid in facedata:
                                        ...
                                        facedata[uid]['phone'] = visitor_data['phone']
                                        if visitor_data.get('email'): facedata[uid]['email'] = visitor_data['email']
                                        if visitor_data.get('address'): facedata[uid]['address'] = visitor_data[
                                            'address']
                                        if visitor_data.get('tahasil'): facedata[uid]['tahasil'] = visitor_data[
                                            'tahasil']
                                        if visitor_data.get('district'): facedata[uid]['district'] = visitor_data[
                                            'district']
                                        if 'visitor' not in facedata[uid] or not isinstance(
                                                facedata[uid].get('visitor'), list):
                                            facedata[uid]['visitor'] = []
                                        facedata[uid]['visitor'].append(visit_entry)
                                    else:
                                        visitor_data['visitor'] = [visit_entry]
                                    ...
                                    if not isinstance(facedata.get(uid), dict):
                                        print(f"Error: Data for UID {uid} is not a dictionary. Aborting save.")
                                        return jsonify(
                                            {'error': 'Internal server error processing data structure.'}), 500
                                ...
                                def search_visitors():
                                    ...
                                    for uid, visitor_data in facedata.items():
                                        if (query in visitor_data.get('name', '').lower() or
                                                query in uid.lower() or
                                                query in visitor_data.get('phone', '').lower() or
                                                any(query in visit.get('visit_id', '').lower() for visit in
                                                    visitor_data.get('visitor', []))):
                                            last_visit = visitor_data.get('visitor', [])[-1] if visitor_data.get(
                                                'visitor') else None
                                            matching_visitors.append({
                                                'uid': uid,
                                                'name': visitor_data.get('name', 'N/A'),
                                ...
                                def get_visitor_details():
                                    ...
                                    if not uid and not visit_id:
                                        return jsonify(
                                            {'success': False, 'message': 'Either uid or visit_id is required'})
                                    try:
                                        with open('data/facedata/facedata.json', 'r') as f:
                                            facedata = json.load(f)
                                    except FileNotFoundError:
                                ...
                                def detect_face():
                                    ...
                                    try:
                                        ...
                                        if person_model is None or not hasattr(person_model, 'predict'):
                                            return jsonify(
                                                {'recognized': False, 'message': 'Model not properly initialized'})
                                        results = person_model.predict(source=frame, stream=True, classes=[0])
                                        max_area, selected_mask = 0, None
                                        for r in results:
                                            if r.boxes is not None and r.masks is not None:
                                                        ...
                                                        for encoding_path in face_encodings_paths:
                                                            if os.path.exists(encoding_path):
                                                                try:
                                                                    ...
                                                                    print(
                                                                        f"Error loading encoding {encoding_path}: {e}")
                                                                    continue
                                                        if not user_encodings:
                                                            images = user.get('images', [])
                                                            encoding_paths = []
                                                            for idx, image_path in enumerate(images):
                                                                ...
                                                                try:
                                                                    ...
                                                                    if not face_encs:
                                                                        continue
                                                                    encoding = face_encs[0]
                                                                    encoding_path = os.path.join('data/encodings',
                                                                                                 f'{uid}_{idx}.npy')
                                                                    os.makedirs('data/encodings', exist_ok=True)
                                                                    np.save(encoding_path, encoding)
                                                                    user_encodings.append(encoding)
                                        ...
                                        return jsonify(
                                            {'recognized': False, 'message': f'Error processing image: {str(e)}'})
                                    finally:
                                        cleanup_memory()
                                ...
                                def confirm_visitor_entry():
                                    ...
                                    if not data:
                                        return jsonify(
                                            {'success': False, 'message': 'Invalid request, no data received'}), 400
                                    visit_id = data.get('visitId')
                                    dvn = data.get('dvn')
                                    if not visit_id or not dvn:
                                        return jsonify(
                                            {'success': False, 'message': 'Visit ID and DVN are required'}), 400
                                    try:
                                        with open('data/facedata/facedata.json', 'r') as f:
                                            facedata = json.load(f)
                                    except FileNotFoundError:
                                    ...
                                    for uid, user_data in facedata.items():
                                        for visit in user_data.get('visitor', []):
                                            if visit.get('visit_id') == visit_id:
                                                visit['dvn'] = dvn
                                                visit['entry_confirmed'] = True
                                                visit['confirmation_time'] = datetime.now().strftime(
                                                    "%Y-%m-%d %H:%M:%S")
                                                with open('data/facedata/facedata.json', 'w') as f:
                                                    json.dump(facedata, f, indent=4)
                                                return jsonify({
                                ...
                                def get_today_visitors():
                                    ...
                                    try:
                                        ...
                                        return jsonify(
                                            {'success': False, 'message': 'No visitor data found', 'visitors': {}})
                                    today_visitors = {}
                                    for uid, user_data in facedata.items():
                                        today_visits = [
                                            visit for visit in user_data.get('visitor', [])
                                ...
                                def process_profile_image():
                                    ...
                                    try:
                                        ...
                                        user_dir = f'data/profiles/{uid}'
                                        os.makedirs(user_dir, exist_ok=True)
                                        existing_images = [f for f in os.listdir(user_dir) if
                                                           f.startswith(f'{uid}_img') and f.endswith('.jpg')]
                                        next_img_num = len(existing_images) + 1
                                        face_path = f'{user_dir}/{uid}_img{next_img_num}.jpg'
                                        cv2.imwrite(face_path, face_crop)
                                        facedata_path = 'data/facedata/facedata.json'
                                        ...
                                        return jsonify(
                                            {'success': False, 'message': f'Error processing image: {str(e)}'}), 500
                                    finally:
                                        cleanup_memory()
                                ...
                                def complete_meeting():
                                    try:
                                        ...
                                        if not visit_found:
                                            return jsonify(
                                                {"error": f"Visit with ID {visit_id} not found for user {uid}"}), 404
                                        with open(json_file_path, 'w') as file:
                                            json.dump(data, file, indent=4)
                                        return jsonify({"success": True,
                                                        "message": f"Meeting {visit_id} marked as completed for user {uid}"}), 200
                                    except Exception as e:
                                        return jsonify({"error": str(e)}), 500
                                ...
                                def visitor_action():
                                    ...
                                    try:
                                        ...
                                        return jsonify(
                                            {"success": False, "message": "Could not load visitor data."}), 500
                                    if uid not in facedata:
                                        return jsonify(
                                            {"success": False, "message": f"Visitor with UID '{uid}' not found."}), 404
                                    visit_found = False
                                    for visit in facedata[uid].get('visitor', []):
                                        if visit.get('visit_id') == visit_id:
                                            visit['remark'] = remark
                                    ...
                                    if not visit_found:
                                        return jsonify({"success": False,
                                                        "message": f"Visit with ID '{visit_id}' not found for visitor '{uid}'."}), 404
                                    try:
                                        with open('data/facedata/facedata.json', 'w') as f:
                                            json.dump(facedata, f, indent=4)
                                        return jsonify(
                                            {"success": True, "message": "Visitor action recorded successfully."}), 200
                                    except Exception as e:
                                        return jsonify({"success": False,
                                                        "message": f"Failed to save updated visitor data: {str(e)}"}), 500
                                        ...
                                        if 'visitor' in user_data:
                                            for visit in user_data['visitor']:
                                                if 'datetime' in visit and isinstance(visit['datetime'], str):
                                                    ...
                                                    print(
                                                        f"Warning: Missing or invalid 'datetime' for visit in user {uid}")
                                    return jsonify({'visitors': today_visitors})
                                    ...
                                    try:
                                        ...
                                        for uid, u in data.items():
                                            ...
                                            try:
                                                ...
                                                continue
                                            enc = any(os.path.exists(p) for p in u.get('face_encodings', []) if
                                                      isinstance(p, str))
                                            img = any(os.path.exists(os.path.join('data', p)) for p in
                                                      u.get('profile_img', []) if isinstance(p, str))
                                            visitors.append(
                                                {"uid": uid, "name": u.get('name', 'N/A'), "has_encodings": enc,
                                                 "has_profile_img": img})
                                        return jsonify(
                                            {"success": True, "visitors": sorted(visitors, key=lambda x: x['name'])})
                                    except Exception as e:
                                        traceback.print_exc()
                                        return jsonify({"success": False, "message": f"Server error: {str(e)}"}), 500
                                ...
                                def upload_visitor_document(visit_id):
                                    ...
                                    file = request.files['document']
                                    if file.filename == '': return jsonify(
                                    ...
                                    try:
                                        ...
                                        found_visit = None
                                        for uid, user_data in face_data.items():
                                            if isinstance(user_data, dict) and 'visitor' in user_data and isinstance(
                                                    user_data['visitor'], list):
                                                for visit in user_data['visitor']:
                                                    if isinstance(visit, dict) and visit.get('visit_id') == visit_id:
                                                        found_visit = visit
                                                        break
                                            if found_visit: break
                                        if not found_visit: return jsonify(
                                            {'success': False, 'error': 'Visitor ID not found'}), 404
                                        existing_pdf_path = found_visit.get('document_pdf')
                                        if existing_pdf_path and isinstance(existing_pdf_path, str) and os.path.exists(
                                                existing_pdf_path):
                                            abs_existing_path = os.path.abspath(existing_pdf_path)
                                            abs_pdf_folder = os.path.abspath(PDF_FOLDER)
                                            if abs_existing_path.startswith(abs_pdf_folder): os.remove(
                                                existing_pdf_path)
                                        from werkzeug.utils import secure_filename
                                        filename = f"{secure_filename(visit_id)}_doc.pdf"
                                        filepath = os.path.join(PDF_FOLDER, filename)
                                        ...
                                        traceback.print_exc()
                                        return jsonify(
                                            {'success': False, 'error': f'An internal error occurred: {str(e)}'}), 500
                                ...
                                def delete_visitor_document(visit_id):
                                    try:
                                        ...
                                        for uid, user_data in face_data.items():
                                            ...
                                            if found_visit: break
                                        if not found_visit: return jsonify(
                                            {'success': False, 'message': 'Visitor ID not found'}), 404
                                        pdf_path_to_delete = found_visit.get('document_pdf')
                                        if not pdf_path_to_delete or not isinstance(pdf_path_to_delete,
                                                                                    str): return jsonify(
                                            {'success': False,
                                             'message': 'No document found associated with this visit entry'}), 404
                                        file_deleted = False
                                        if os.path.exists(pdf_path_to_delete):
                                            try:
                                                abs_delete_path = os.path.abspath(pdf_path_to_delete)
                                                ...
                                                return jsonify(
                                                    {'success': False, 'message': f'Error deleting file: {e}'}), 500
                                        else:
                                            file_deleted = True
                                        if file_deleted:
                                            found_visit['document_pdf'] = None
                                            if save_face_data(face_data):
                                                ...
                                                return jsonify({'success': False,
                                                                'message': 'File deleted but failed to update visitor record.'}), 500
                                        else:
                                            return jsonify({'success': False,
                                                            'message': 'File deletion was prevented or failed.'}), 500
                                    except Exception as e:
                                        import traceback;
                                        traceback.print_exc()
                                        return jsonify({'success': False,
                                                        'message': f'An internal server error occurred: {str(e)}'}), 500
                                ...
                                def get_visitor_document(visit_id):
                                    ...
                                    if pdf_filename:
                                        try:
                                            print(
                                                f"Attempting to send file: Directory='{PDF_FOLDER}', Filename='{pdf_filename}'")
                                            return send_from_directory(
                                                PDF_FOLDER,
                                                pdf_filename,
                                                as_attachment=False,
                                            ...
                                            print(
                                                f"Error: File not found at expected location: {os.path.join(PDF_FOLDER, pdf_filename)}")
                                            return jsonify({'error': 'Document file not found on server.'}), 404
                                        except Exception as e:
                                            print(f"Error sending file: {e}")
                                            return jsonify(
                                                {'error': 'An error occurred while retrieving the document.'}), 500
                                    else:
                                        print(f"Document reference not found in JSON data for visit_id: {visit_id}")
                                        return jsonify(
                                            {'error': 'Document reference not found for this visitor ID.'}), 404
                                ...
                                def generate_pdf():
                                    if 'images' not in request.json or 'visit_id' not in request.json:
                                        return jsonify(
                                            {'success': False, 'error': 'No images or visit ID provided'}), 400
                                    image_paths_to_delete = []
                                    try:
                                        image_ids = request.json['images']
                                        visit_id = request.json['visit_id']
                                        if not visit_id: return jsonify(
                                            {'success': False, 'error': 'Visit ID is missing'}), 400
                                        if not image_ids: return jsonify({'success': False,
                                                                          'error': 'No image IDs provided for PDF generation'}), 400
                                        from werkzeug.utils import secure_filename
                                        pdf_filename = f"{secure_filename(visit_id)}_doc.pdf"
                                        pdf_path = os.path.join(PDF_FOLDER, pdf_filename)
                                        if os.path.exists(pdf_path):
                                        ...
                                        if updated:
                                            if save_face_data(face_data): pass
                                        return jsonify({'success': True,
                                                        'message': 'PDF generated successfully from scanned images.',
                                        ...
                                        try:
                                            ...
                                            pass
                                        return jsonify(
                                            {'success': False, 'error': f'Failed to generate PDF: {str(e)}'}), 500
                                ...
                                def load_departments():
                                    try:
                                        ...
                                        with open(DEPARTMENTS_FILE, 'r', encoding='utf-8') as f:
                                            ...
                                            departments = data.get('departments', [])
                                            if isinstance(departments, list):
                                                valid_departments = [d for d in departments if
                                                                     isinstance(d, dict) and d.get('name') and d.get(
                                                                         'email')]
                                                if len(valid_departments) != len(departments):
                                                    print(
                                                        "Warning: Some entries in departments.json might be missing 'name' or 'email'.")
                                                return valid_departments
                                            else:
                                                print(
                                                    f"Error: 'departments' key in {DEPARTMENTS_FILE} does not contain a list.")
                                                return []
                                    except json.JSONDecodeError as e:
                                        print(f"Error: Invalid JSON format in {DEPARTMENTS_FILE}: {e}")
                                        return []
                                ...
                                def login_department():
                                    ...
                                    if request.method == 'POST':
                                        ...
                                        department = next(
                                            (d for d in departments_list if d.get('name') == selected_department_name),
                                            None)
                                        ...
                                        if department:
                                            ...
                                            if send_otp_email(email, otp):
                                                flash(
                                                    f'An OTP has been sent to the registered email for {selected_department_name}. Please check the inbox.',
                                                    'info')
                                                return redirect(url_for('verify_otp'))
                                            else:
                                                session.pop('otp', None)
                                                ...
                                                session.pop('pending_department_info', None)
                                                flash(
                                                    'Failed to send OTP email. Please check server logs or contact support.',
                                                    'danger')
                                                return render_template('department/auth/login_department.html',
                                                                       departments=departments_list)
                                        else:
                                ...
                                def verify_otp():
                                    pending_info = session.get('pending_department_info')
                                    display_email = pending_info.get('email',
                                                                     'your registered email') if pending_info else 'your registered email'
                                    department_name = pending_info.get('name',
                                                                       'Your Department') if pending_info else 'Your Department'
                                    if request.method == 'POST':
                                        entered_otp = request.form.get('otp')
                                        stored_otp = session.get('otp')
                                        otp_timestamp = session.get('otp_timestamp')
                                        if not entered_otp:
                                            ...
                                            flash('OTP has expired (valid for 5 minutes). Please request a new one.',
                                                  'danger')
                                            session.pop('otp', None)
                                            session.pop('otp_timestamp', None)
                                            session.pop('pending_department_info', None)
                                            ...
                                            flash('Invalid OTP entered. Please try again.', 'danger')
                                    return render_template('department/auth/verify_otp.html', email=display_email,
                                ...
                                def department_dashboard():
                                    department = session.get('department')
                                    return render_template('department/dashboard/department_dashboard.html',
                                    ...
                                    for uid, user_data in facedata.items():
                                        for visit in user_data.get('visitor', []):
                                            if visit.get('forwarding_department') == department_name:
                                                ...
                                                if start_date or end_date:
                                                    try:
                                                        visit_date = datetime.strptime(visit.get('datetime'),
                                                                                       '%Y-%m-%d %H:%M:%S').date()
                                                        if start_date:
                                                            start = datetime.strptime(start_date, '%Y-%m-%d').date()
                                                            if visit_date < start:
                                                ...
                                                application = {
                                                    ...
                                                    'profile_img': user_data.get('profile_img', [''])[
                                                        0] if user_data.get('profile_img') else ''
                                                }
                                                applications.append(application)
                                        ...
                                        for i, visit in enumerate(user_data.get('visitor', [])):
                                            if visit.get('visit_id') == visit_id:
                                                if visit.get('forwarding_department') == current_department_name:
                                                    ...
                                                    user_data['visitor'][i]['forwarded_from'] = current_department_name
                                                    user_data['visitor'][i][
                                                        'forwarded_datetime'] = datetime.now().strftime(  # Corrected
                                                        '%Y-%m-%d %H:%M:%S')
                                                    updated = True
                                                    break
                                        if updated:
                                ...
                                def get_admin_visitors():
                                    ...
                                    for uid, user_data in face_data.items():
                                        if isinstance(user_data, dict) and 'visitor' in user_data and isinstance(
                                                user_data['visitor'], list):
                                            for visit in user_data['visitor']:
                                                if isinstance(visit, dict) and 'datetime' in visit and isinstance(
                                                        visit['datetime'], str):
                                                    try:
                                                        if visit['datetime'].split(' ')[0] == selected_date_str:
                                                            daily_visitors.append({
                                                                ...
                                                                "entry_confirmed": visit.get('entry_confirmed', False),
                                                                "dvn": visit.get('dvn', None),
                                                                "forwarding_department": visit.get(
                                                                    'forwarding_department', ''),
                                                                "forwarding_note": visit.get('forwarding_note', ''),
                                                                # Added forwarding_note
                                                                "remark": visit.get('remark', ''),
                                                                # Added remark explicitly
                                                                "document_pdf": visit.get('document_pdf', None)
                                                            })
                                                    except Exception as e:
                                                        # Log errors during processing individual visits if needed
                                                        print(
                                                            f"Error processing visit for UID {uid}, Visit {visit.get('visit_id', 'N/A')}: {e}")
                                                else:
                                                    # Log or handle invalid visit entries
                                                    print(
                                                        f"Warning: Skipping invalid visit entry for UID {uid}: {visit}")
                                        else:
                                            # Log or handle invalid user data structures
                                            print(f"Warning: Skipping invalid user data structure for UID {uid}")
                                    ...
                                    try:
                                        ...
                                        valid_visitors = [v for v in daily_visitors if
                                                          'datetime' in v and isinstance(v['datetime'], str)]
                                        invalid_visitors = [v for v in daily_visitors if
                                                            not ('datetime' in v and isinstance(v['datetime'], str))]
                                ...
                                def delete_visitor_entry():
                                    ...
                                    if not uid or not visit_id:
                                        return jsonify(
                                            {"success": False, "message": "Missing or empty uid/visit_id"}), 400
                                    ...
                                    try:
                                        ...
                                        if not isinstance(user_visits, list):
                                            return jsonify(
                                                {"success": False, "message": "Invalid or missing visitor data"}), 404
                                        visit_to_remove = next((v for v in user_visits if
                                                                isinstance(v, dict) and v.get('visit_id') == visit_id),
                                                               None)
                                        if not visit_to_remove:
                                            return jsonify({"success": False,
                                                            "message": f"Visit ID '{visit_id}' not found for UID '{uid}'"}), 404
                                        face_data[uid]['visitor'] = [v for v in user_visits if
                                                                     v.get('visit_id') != visit_id]
                                        pdf = visit_to_remove.get('document_pdf')
                                        if pdf and isinstance(pdf, str) and pdf.strip():
                                            abs_path, abs_folder = os.path.abspath(pdf), os.path.abspath(PDF_FOLDER)
                                            if abs_path.startswith(abs_folder) and os.path.exists(pdf):
                                        ...
                                        if save_face_data(face_data):
                                            return jsonify(
                                                {"success": True, "message": "Visitor entry deleted successfully"})
                                        return jsonify({"success": False, "message": "Failed to save changes"}), 500
                                    except Exception as e:
                                        print(f"Error deleting visit {visit_id} for UID {uid}: {e}")
                                        return jsonify({"success": False, "message": f"Server error: {str(e)}"}), 500
                                ...
                                def generate_visitor_report():
                                    ...
                                    try:
                                            ...
                                            if isinstance(user_data, dict) and 'visitor' in user_data and isinstance(
                                                ...
                                                for visit in user_data['visitor']:
                                                    if isinstance(visit, dict) and 'datetime' in visit and isinstance(
                                                            visit['datetime'], str):
                                                        try:
                                                            visit_dt = datetime.strptime(visit['datetime'],
                                                                                         '%Y-%m-%d %H:%M:%S')
                                                            visit_date = visit_dt.date()
                                                            if start_date <= visit_date <= end_date:
                                                                report_entry = {
                                                                    ...
                                                                    "District": safe_get(user_data, 'district'),
                                                                    "Visit ID": safe_get(visit, 'visit_id'),
                                                                    "Date & Time": visit_dt.strftime(
                                                                        '%d/%m/%Y %I:%M:%S %p'),
                                                                    "Purpose": safe_get(visit, 'purpose'),
                                                                    "Document": "Yes" if visit.get(
                                                                        'document_pdf') else "No",
                                                                    "Status": safe_get(visit, 'status',
                                                                                       'unknown').capitalize(),
                                                                    "Entry Confirmed": "Yes" if visit.get(
                                                                        'entry_confirmed', False) else "No",
                                                                    "Forwarded To": safe_get(visit,
                                                                                             'forwarding_department',
                                                                                             '-'),
                                                                    "Remark": safe_get(visit, 'forwarding_note', '-')
                                                                }
                                        ...
                                        report_data.sort(
                                            key=lambda x: datetime.strptime(x['Date & Time'], '%d/%m/%Y %I:%M:%S %p'))
                                        ...
                                        ws.append(headers)
                                        header_font = Font(bold=True, color="FFFFFF")
                                        header_fill = openpyxl.styles.PatternFill(start_color="2C3E90",
                                                                                  end_color="2C3E90", fill_type="solid")
                                        for cell in ws[1]:
                                            cell.font = header_font
                                            cell.fill = header_fill
                                            cell.alignment = Alignment(horizontal='center', vertical='center')
                                        ...
                                        for col_idx, column_letter in enumerate(
                                                openpyxl.utils.get_column_letter(i) for i in
                                        ...
                                        print(f"CRITICAL: Error generating Excel report: {e}")
                                        return jsonify({
                                                           "error": f"An unexpected server error occurred while generating the report: {str(e)}"}), 500
                                ...
                                def delete_user():
                                    ...
                                    if not data or 'uid' not in data:
                                        return jsonify(
                                            {"success": False, "message": "Missing UID in request body"}), 400
                                    uid_to_delete = data['uid']
                                    try:
                                        face_data = load_face_data()
                                        if uid_to_delete not in face_data:
                                            return jsonify({"success": False,
                                                            "message": f"Visitor UID '{uid_to_delete}' not found"}), 404
                                        removed_data = face_data.pop(uid_to_delete, None)
                                        if removed_data is None:
                                            return jsonify({"success": False,
                                                            "message": "User UID found initially but could not be removed."}), 500
                                        encodings_dir = 'data/encodings'
                                        if isinstance(removed_data, dict) and 'face_encodings' in removed_data:
                                            for encoding_path in removed_data.get('face_encodings', []):
                                                if encoding_path and isinstance(encoding_path, str):
                                                    full_encoding_path = os.path.abspath(encoding_path)
                                                    expected_dir = os.path.abspath(encodings_dir)
                                                    if full_encoding_path.startswith(expected_dir) and os.path.exists(
                                                            full_encoding_path):
                                                        try:
                                                            os.remove(full_encoding_path)
                                                        except OSError as e:
                                                            print(
                                                                f"Error deleting encoding file {full_encoding_path}: {e}")
                                                    elif os.path.exists(encoding_path):
                                                        try:
                                                            os.remove(encoding_path)
                                                        except OSError as e:
                                            ...
                                            for visit in removed_data.get('visitor', []):
                                                if isinstance(visit, dict) and 'document_pdf' in visit:
                                                    ...
                                                    if pdf_path and isinstance(pdf_path, str):
                                                        abs_pdf_path = os.path.abspath(pdf_path)
                                                        abs_pdf_folder = os.path.abspath(PDF_FOLDER)
                                                        if abs_pdf_path.startswith(abs_pdf_folder) and os.path.exists(
                                                                abs_pdf_path):
                                                            try:
                                                                os.remove(abs_pdf_path)
                                                            except OSError as e:
                                                                print(
                                                                    f"Error deleting PDF {abs_pdf_path} during user deletion: {e}")
                                                        elif os.path.exists(pdf_path):
                                                            try:
                                                                os.remove(pdf_path)
                                                            except OSError as e:
                                                                print(
                                                                    f"Error deleting PDF {pdf_path} during user deletion: {e}")
                                        save_face_data(face_data)
                                        return jsonify({"success": True,
                                                        "message": f"User '{uid_to_delete}' and associated files deleted successfully"})
                                    except Exception as e:
                                        print(f"Error deleting user {uid_to_delete}: {e}")
                                        import traceback
                                        traceback.print_exc()
                                        return jsonify({"success": False,
                                                        "message": f"An server error occurred during user deletion: {str(e)}"}), 500
